...
def main():
    ...
    # Load samples csv
    msh_samples_data_set = pd.read_csv(
        "/home/daraghhollman/Main/Work/mercury/DataSets/magnetopause/magnetosheath_sample_10_mins.csv",
        parse_dates=["Crossing Start", "Crossing End"],
    )
    msp_samples_data_set = pd.read_csv(
        "/home/daraghhollman/Main/Work/mercury/DataSets/magnetopause/magnetosphere_sample_10_mins.csv",
        parse_dates=["Crossing Start", "Crossing End"],
    )
    ...
    for samples_data_set, label, output in zip(
        [msh_samples_data_set, msp_samples_data_set], ["MSh", "MSp"], outputs
    ):
...
def Get_Features(input):
    ...
    try:
        ...
        if label == "MSh":
            ...
            # Magnetosphere sample is before the crossing
            if crossing_start > sample_start:
                is_inbound = 0

            # Magnetosphere sample is after the crossing
            else:
                is_inbound = 1

        else:
            raise Exception(f"Unknown sample label: {label}, expected 'MSh' or 'MSp'")
...
def Get_Grazing_Angle(row, function="magnetopause"):
    """
